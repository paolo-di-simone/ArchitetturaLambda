...
from pyspark.sql.functions import collect_list, create_map, sum, udf
...
http_response_codes = [
    "100", "101", "200", "201", "202", "204", "206", "300", "301", "302",
    "304", "307", "308", "400", "401", "403", "404", "405", "406", "409",
    "410", "413", "415", "429", "500", "501", "502", "503", "504", "505"
]

def extract_response_code(payload):
	if payload != None:
		for type in http_response_codes:
			if type in payload:
				return type
	return None

extract_response_code_udf = udf(extract_response_code, StringType())


################################################ ELABORAZIONE CON SPARK STREAMING ################################################


parsed_df = parsed_df.withColumn("response_code", extract_response_code_udf(col("payload")))

windowed_df = parsed_df \
	.withWatermark("timestamp", kafka_parameters["WINDOW_DURATION"]) \
	.groupBy(window("timestamp", kafka_parameters["WINDOW_DURATION"]), "response_code") \
    .count()

windowed_df = windowed_df.filter(col("response_code").isNotNull())

windowed_df = windowed_df.withColumnRenamed("count", "n_packets_by_http_response_code")
...
def write_to_influxdb(row):
	...
	try:
		...
		point.tag("response_code", row.response_code)
		point.field("n_packets_by_http_response_code", row.n_packets_by_http_response_code)

		write_api.write(bucket=influxdb_parameters["BUCKET"], org=influxdb_parameters["ORG"], record=point)

		print("Oggetto point")
		print("Timestamp:", timestamp)
		print("Response code:", row.response_code)
		print("N packets:", row.n_packets_by_http_response_code)
		print()
