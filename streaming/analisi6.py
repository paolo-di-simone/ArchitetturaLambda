...
from pyspark.sql.functions import collect_list, create_map, sum, udf, array, sort_array
...
parsed_df = parsed_df.filter(col("flag_tcp") == "S")

ip_pair_col = array(parsed_df["ip_src"], parsed_df["ip_dst"])
sorted_ip_pair_col = sort_array(ip_pair_col)

parsed_df = parsed_df.withColumn("ip_pair", sorted_ip_pair_col)

windowed_df = parsed_df \
	.withWatermark("timestamp", kafka_parameters["WINDOW_DURATION"]) \
	.groupBy(window("timestamp", kafka_parameters["WINDOW_DURATION"]), "ip_pair") \
    .count()

windowed_df = windowed_df.withColumnRenamed("count", "n_connections")
...
def write_to_influxdb(row):
	...
	try:
		...
		point.tag("ip_pair", row.ip_pair)
		point.field("n_connections", row.n_connections)

		write_api.write(bucket=influxdb_parameters["BUCKET"], org=influxdb_parameters["ORG"], record=point)

		print("Oggetto point")
		print("Timestamp:", timestamp)
		print("IP pair:", row.ip_pair)
		print("N connections:", row.n_connections)
		print()
