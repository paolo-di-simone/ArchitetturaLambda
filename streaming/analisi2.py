...
windowed_df = parsed_df \
	...
	.groupBy(window("timestamp", kafka_parameters["WINDOW_DURATION"]), "ip_dst") \
	.count()

windowed_df = windowed_df.withColumnRenamed("count", "n_packets_by_ip_dst")
...
def write_to_influxdb(row):
	...
	try:
		...
		point.tag("ip_dst", row.ip_dst)
		point.field("n_packets_by_ip_dst", row.n_packets_by_ip_dst)
		...
		print("IP:", row.ip_dst)
		print("N packets:", row.n_packets_by_ip_dst)
		print()
