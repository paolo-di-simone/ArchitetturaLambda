...
http_request_types = ["GET", "POST", "PUT", "DELETE", "HEAD", "PATCH", "OPTIONS", "TRACE", "CONNECT"]

def extract_request_type(payload):
	if payload != None:
		for type in http_request_types:
			if type in payload:
				return type
	return None

extract_request_type_udf = udf(extract_request_type, StringType())


################################################ ELABORAZIONE CON SPARK STREAMING ################################################


parsed_df = parsed_df.withColumn("request_type", extract_request_type_udf(col("payload")))

windowed_df = parsed_df \
	.withWatermark("timestamp", kafka_parameters["WINDOW_DURATION"]) \
	.groupBy(window("timestamp", kafka_parameters["WINDOW_DURATION"]), "request_type") \
    .count()

windowed_df = windowed_df.filter(col("request_type").isNotNull())

windowed_df = windowed_df.withColumnRenamed("count", "n_packets_by_http_request_type")
...
def write_to_influxdb(row):
	...
	try:
		...
		point.tag("request_type", row.request_type)
		point.field("n_packets_by_http_request_type", row.n_packets_by_http_request_type)

		write_api.write(bucket=influxdb_parameters["BUCKET"], org=influxdb_parameters["ORG"], record=point)

		print("Oggetto point")
		print("Timestamp:", timestamp)
		print("Request type:", row.request_type)
		print("N packets:", row.n_packets_by_http_request_type)
		print()
